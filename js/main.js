...
(function() {
  var ASSETS, AimPlayerMover, BULLET_IMG, Bullet, Enemy, Material, Mover, PLAYER_IMG, Player, RadialShooter, Shooter, StraightMover, add, enemies, enemy_bullets, game, intoWindow, isInWindow, normalize, p, player, player_bullets, players, remove,
  ...
  AimPlayerMover = (function(_super) {
    ...
    AimPlayerMover.prototype["do"] = function(material) {
      ...
      _ref = normalize(material.rx - player.rx, material.ry - player.ry), vx = _ref[0], vy = _ref[1];
      p("" + vx + ", " + vy);
  ...
  RadialShooter = (function(_super) {
    __extends(RadialShooter, _super);

    function RadialShooter(density) {
      this.density = density;
    }

    RadialShooter.prototype["do"] = function(material) {
      var angle, i, vx, vy, _i, _ref, _results;

      angle = material.age;
      _results = [];
      for (i = _i = 1, _ref = this.density; 1 <= _ref ? _i <= _ref : _i >= _ref; i = 1 <= _ref ? ++_i : --_i) {
        vx = Math.cos(angle) * 2;
        vy = Math.sin(angle) * 2;
        new Bullet(56, material.rx, material.ry, vx, vy, enemy_bullets);
        _results.push(angle += Math.PI * 2 / this.density);
      }
      return _results;
    };

    return RadialShooter;
    ...
    game.onload = function() {
      ...
      scene.onenterframe = function() {
        ...
        if (game.frame % (game.fps * 1) === 0) {
          ...
          return e.mover = new AimPlayerMover(-2, 2);
        }
      };
      bex = bey = 0;
