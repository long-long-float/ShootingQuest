...
(function() {
  var ASSETS, AimBullet, AimStraightMover, AimStraightShooter, BULLET_IMG, Bullet, CountLabel, Enemy, FLOOR_IMG, Gauge, MAP_IMG, MOVE_VEROCITY, Material, Mover, PLAYER_IMG, Player, Shooter, ShotShooter, StraightMover, StraightShooter, add, bind_new, enemies, enemy_bullets, exp, exp_gauge, game, increment_level, intoWindow, isInWindow, leapX, leapY, level, level_label, normalize, player, player_bullets, players, puts, rand, remove, to_angle, to_angle_material, to_vec, var_dump,
    __slice = [].slice,
  ...
  ASSETS = [PLAYER_IMG = 'player.png', BULLET_IMG = 'icon0.png', MAP_IMG = 'map1.png', FLOOR_IMG = 'floor.png'];
  ...
  normalize = function() {
    var i, len, sum, _i, _j, _len, _len1, _results;
    sum = 0;
    for (_i = 0, _len = arguments.length; _i < _len; _i++) {
      i = arguments[_i];
      sum += i * i;
    ...
    Player.prototype.onenterframe = function() {
      ...
      if (game.frame % (game.fps / 15) === 0) {
        ...
        for (i = _i = -n; -n <= n ? _i <= n : _i >= n; i = -n <= n ? ++_i : --_i) {
          _results.push(new Bullet(this.rx + (this.width / 4) * i, this.ry - this.height / 2, 0, -1, player_bullets, 10, 48));
        }
        return _results;
      }
    };
  ...
  AimStraightMover = (function(_super) {
    ...
    function AimStraightMover(parent, v, fixed) {
      ...
      this.fixed = fixed;
      if (this.fixed) {
        _ref = normalize(player.rx - this.parent.rx, player.ry - this.parent.ry).map((function(_this) {
          return function(v) {
            return v * _this.v;
          };
        })(this)), this.parent.vx = _ref[0], this.parent.vy = _ref[1];
      } else {
        this.parent.vy = 1;
      }
    }
    ...
    AimStraightMover.prototype["do"] = function() {
      var angle, goal_angle, now_angle, _ref;
      if (!this.fixed) {
        ...
        return _ref = to_vec(angle).map((function(_this) {
          return function(v) {
            return v * _this.v;
          };
  ...
  AimBullet = (function(_super) {
    ...
    AimBullet.prototype.onenterframe = function() {
      ...
      if (this.age === game.fps) {
        _ref = normalize(player.rx - this.rx, player.ry - this.ry).map((function(_this) {
          return function(v) {
            return v * Math.sqrt(_this.vx * _this.vx + _this.vy * _this.vy) * 2;
          };
        })(this)), this.vx = _ref[0], this.vy = _ref[1];
  ...
  Leap.loop(function(frame) {
    ...
    if (frame.fingers.length > 0) {
      _ref = frame.fingers[0].tipPosition, leapX = _ref[0], leapY = _ref[1];
      return console.log(leapY);
    }
  });
  ...
  window.onload = function() {
    ...
    game.onload = function() {
      var bex, bey, floor, h, movers, positions, scene, shooters, size, w, x, y, _i, _j, _ref, _ref1;
      scene = game.rootScene;
      scene.backgroundColor = '#808080';
      size = 320;
      for (y = _i = -1, _ref = game.height / size; -1 <= _ref ? _i <= _ref : _i >= _ref; y = -1 <= _ref ? ++_i : --_i) {
        for (x = _j = 0, _ref1 = game.width / size + 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; x = 0 <= _ref1 ? ++_j : --_j) {
      ...
      scene.onenterframe = function() {
        var e, first, group, group_set, group_sets, i, m, mover, p, pos, second, v, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _n, _o, _p, _ref2, _ref3, _ref4, _ref5, _ref6;
        _ref2 = [players, enemies, player_bullets, enemy_bullets];
        for (_k = 0, _len = _ref2.length; _k < _len; _k++) {
          group = _ref2[_k];
          _ref3 = group.childNodes;
        ...
        for (_m = 0, _len2 = group_sets.length; _m < _len2; _m++) {
          ...
          _ref4 = group_set[0].childNodes;
          for (_n = 0, _len3 = _ref4.length; _n < _len3; _n++) {
            first = _ref4[_n];
            _ref5 = group_set[1].childNodes;
        ...
        if (game.frame % (game.fps * 2) === 0) {
          for (i = _p = 1, _ref6 = Math.max(level, 2); 1 <= _ref6 ? _p <= _ref6 : _p >= _ref6; i = 1 <= _ref6 ? ++_p : --_p) {
